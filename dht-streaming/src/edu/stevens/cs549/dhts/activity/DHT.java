...
//		routing.joinCheck();
			...
			if (IRouting.USE_FINGER_TABLE) {
				for(int i= IRouting.NFINGERS-1;i>=1;i--){
					...
					try{
						...
						System.err.println("the uri is wrong: "+e);
						}
					}
				}
				//return info;
	...
	public void putRequest(String uri) {
		...
		client.putRequest(uris, Entity.text(""));
			 ...
			 System.err.println("Put request failed" + ex);
			ex.printStackTrace();
		}
		}
	public void deleteRequest(String uri) {
		...
		client.deleteRequest(uris);
			...
			System.err.println("delete request failed " + ex);
		ex.printStackTrace();
		}
		}
	...
	public EventOutput listenForBindings(int id, String key) {
		...
		EventOutput eout = new EventOutput();
		state.addListener(id, key, eout);
		return eout;
	}
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		...
		int id = NodeKey(key);
		NodeInfo succ = this.findSuccessor(id);
		EventSource eventsrc = client.listenForBindings(succ, info.id, key);
		state.addCallback(key, eventsrc);
		eventsrc.register(listener);
		eventsrc.open();
	...
	public void listenOff(String key) throws DHTBase.Failed {
		...
		int id = NodeKey(key);
		NodeInfo succ = this.findPredecessor(id);
