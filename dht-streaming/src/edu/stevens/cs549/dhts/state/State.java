...
public class State implements IState, IRouting {
	...
	@Override
	public void addListener(int id,String key,EventOutput eventopt) {
		if(outputs.containsKey(id)){
			outputs.get(id).put(key, eventopt);
		} else {
			Map<String,EventOutput> map = new HashMap<String,EventOutput>();
			map.put(key,eventopt);
			outputs.put(id,map);
		}
		
		if(listeners.containsKey(key)){
			listeners.get(key).add(eventopt);
		} else {
			Map<String,EventOutput> map = new HashMap<String,EventOutput>();
	...
	public void removeListener(int id, String key) {
		...
		if(listeners.containsKey(key)){
			listeners.get(key).remove(outputs.get(id).get(key));
		}
	...
	private void broadcastAddition(String key, String value) {
		...
		if(listeners.containsKey(key)) {
			...
			OutboundEvent evt = builder.name(IDHTNode.NEW_BINDING_EVENT).data(String.class,value).build();
			listeners.get(key).broadcast(evt);
		}
	}
	...
	public void removeCallback(String key) {
		...
		EventSource eventsrc = callbacks.get(key);
		if (eventsrc != null) {
			eventsrc.close();
